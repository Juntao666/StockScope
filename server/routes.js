...
const author = async function(req, res) {
  ...
  const name = 'Zhujie Xu';
  const pennkey = 'xu1573';
...
const search_songs = async function(req, res) {
  ...
  const valLow = req.query.valence_low ?? 0;
  const valHigh = req.query.valence_high ?? 1;
...
const entrance_songs = async function(req, res) {
  ...
  const values = [maxEnergy, minDanceability, limit];
  connection.query(query, values, (err, data) => {
    if (err) {
      console.log(err);
      res.json([]);
